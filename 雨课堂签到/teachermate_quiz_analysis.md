...
> [!NOTE]
> **关于正确答案**：当 `answerOpen=1` 且题目已关闭 (`status=2`) 时，`answerContent` 中的 `answer` 字段会标记正确选项为 `true`。但从 HAR 抓包来看，所有选项的 `answer` 都是 `false`——这**很可能是因为该学生未作答 (`isAnswered=0`)**，答案只有在作答后才会显示。
...
```json
{
  "courseId": 1454547,
  "questionId": 34317596,
  "answer": [0],           // 选择题: 选中选项的 rank 数组; 填空题: 文本数组
...
|---------|------|-------------|-------------|
| 0 | 单选题 | 1 | `[rank]` — 单个 rank 值的数组 |
| 1 | 单选题（变体） | 1 | `[rank]` |
| 2 | 多选题 | 1 | `[rank1, rank2, ...]` — 多个 rank |
| 3 | 填空题 | 1 | `["答案1", "答案2", ...]` — 每个空一个字符串 |
| 4 | 判断题 | 1 | `[0]` 或 `[1]` — 0=某个选项, 1=另一个 |
...
sequenceDiagram
    ...
    App->>TM: 20. POST /wechat-api/v3/students/answer/question {courseId, questionId, answer:[2]}
    TM-->>App: 21. 返回提交结果
