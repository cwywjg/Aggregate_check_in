<USER_REQUEST>
还有几个问题，一个是# 微助教（TeacherMate）全协议签到技术实现规范（增强版）
...
| **Faye WebSocket**| `WSS` | `wss://www.teachermate.com.cn/faye` | 订阅 `/attendance/{courseId}/{signId}/qr` |    我们要完美加入这些打卡机制，正常签到一般用不到，就设置个按钮就行了，点击直接签到，然后gps签到，在主页面加上经纬度配置就行了，然后给app加个一键获取当前详细经纬度的功能，方便配置，然后扫码签到也需要优化，可以做成两个模式，一个是自动监听，自动解析二维码，自动签到的，一个是保底的，手动扫码批量签到，一键开启关闭，   可以有个总开关，打开就自动探测签到事件，然后判断签到类型进行自动或手动签到

</USER_REQUEST>
<ADDITIONAL_METADATA>
The current local time is: 2026-08-22T07:57:16+08:00.
</ADDITIONAL_METADATA>