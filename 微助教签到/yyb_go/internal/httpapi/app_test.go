...
func TestRefreshLockForReturnsStablePerAccountLock(t *testing.T) {
	app := &App{}
	first := app.refreshLockFor(7)
	second := app.refreshLockFor(7)
	other := app.refreshLockFor(8)
	if first != second {
		t.Fatal("same account must share one refresh lock")
	}
	if first == other {
		t.Fatal("different accounts must use independent refresh locks")
	}
}
...
func TestHandlerServesGinRoutesAndSwaggerDocs(t *testing.T) {
	...
	if healthBody.Data["session_ttl_seconds"] != float64(60) {
		t.Fatalf("GET /health session_ttl_seconds = %#v", healthBody.Data["session_ttl_seconds"])
	}
